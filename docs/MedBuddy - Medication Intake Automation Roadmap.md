...
- 초기 격자는 4 x 3을 요청하며 기본 높이 240dp와 최소 축소 높이 180dp를 구분한다.
  `DoseWidgetLayout`은 실제 배정 높이와 글자 배율에 맞춰 여백·상세 정보량을 조절한다.
...
- 로그인 정보가 없으면 복약 정보 대신 `로그인이 필요해요`와 `로그인하기`를 표시한다.
  이 상태에서는 진행률·새로고침을 숨기고 로그인 화면만 연다. 로그인 뒤 일정이 없는
  상태나 조회 실패와 구분하며, 로그인 버튼으로 복용 기록을 만들지 않는다.
