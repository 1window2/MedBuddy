...
# Step 3 : Google Generative AI를 초기화한다.
genai.configure(api_key=gemini_key)

# Step 4 : 애플리케이션 실행 시 필요한 데이터베이스 테이블을 생성한다.
db_models.Base.metadata.create_all(bind=database.engine)

# Step 5 : FastAPI 애플리케이션 인스턴스를 생성한다.
app = FastAPI(title="MedBuddy API", version="1.0.0")

# Step 6 : 약품 관련 라우터를 애플리케이션에 등록한다.
app.include_router(medication_router, prefix="/api/v1/medication", tags=["Medication"])

# Step 7 : 현재 파일이 직접 실행된 경우 Uvicorn 서버를 실행한다.
