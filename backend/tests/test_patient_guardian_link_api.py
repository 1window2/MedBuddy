...
from api.dependencies import get_patient_guardian_link_control  # noqa: E402
from api.router import router  # noqa: E402
from controls.patient_guardian_link_control import (  # noqa: E402
    PatientGuardianLinkControl,
)
from core.database import Base  # noqa: E402


def test_patient_and_guardian_clients_complete_link_lifecycle() -> None:
    ...
    def override_link_control() -> Generator[PatientGuardianLinkControl, None, None]:
        db: Session = session_factory()
        try:
            yield PatientGuardianLinkControl(db=db)
        finally:
            db.close()

    app = FastAPI()
    app.include_router(router, prefix="/api/v1/medication")
    app.dependency_overrides[get_patient_guardian_link_control] = (
    ...
    async def run_link_lifecycle() -> None:
        async with (
            ...
            httpx.AsyncClient(
                transport=httpx.ASGITransport(app=app),
                base_url="http://guardian-device.test",
            ) as guardian_device,
        ):
            ...
            register_response = await guardian_device.post(
                "/api/v1/medication/link/register",
                json={
                    "guardian_hash": "guardian-device",
            ...
            assert link["guardian_hash"] == "guardian-device"
            ...
            guardian_links = await guardian_device.get(
                "/api/v1/medication/link/list",
                params={"user_hash": "guardian-device"},
            )
            assert patient_links.status_code == 200
            assert guardian_links.status_code == 200
            assert patient_links.json()["data"] == [link]
            assert guardian_links.json()["data"] == [link]

            unlink_response = await guardian_device.delete(
            ...
            guardian_links_after_unlink = await guardian_device.get(
                "/api/v1/medication/link/list",
                params={"user_hash": "guardian-device"},
            )
            assert patient_links_after_unlink.status_code == 200
            assert guardian_links_after_unlink.status_code == 200
            assert patient_links_after_unlink.json()["data"] == []
            assert guardian_links_after_unlink.json()["data"] == []
