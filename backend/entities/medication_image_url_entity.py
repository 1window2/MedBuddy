...
# Role: Validates medication image URLs against the trusted MFDS HTTPS origin before storage or display.
...
# Description:
# - Normalizes a public medication image URL and returns it only when it uses HTTPS on the documented MFDS image host.
# - Rejects credentials, alternate ports, local endpoints, and arbitrary third-party tracking hosts before the value is stored or returned.
# Parameters:
# - value (object): Untrusted URL value received from a client, catalog, or database.
# Returns:
